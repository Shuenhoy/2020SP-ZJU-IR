...
inline bool is_token(const char& c, bool keep_asterisk) {
    return std::isalnum(c) || c == '_' || c == '-' || keep_asterisk && c == '*';
}

/* 将字符串变成词项，目前仅仅保留了字母、数字、下划线与连词符，其他符号全部去除 */
inline std::vector<std::string_view> tokenize(std::string_view input, bool keep_asterisk) {
    std::vector<std::string_view> tokens;
    for (size_t i = 0; i < input.size(); i++) {
        if (is_token(input[i], keep_asterisk)) {
            size_t begin = i;
            while (++i < input.size() && is_token(input[i], keep_asterisk))
                ;
